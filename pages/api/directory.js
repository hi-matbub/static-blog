...
export default (req, res) => {
  ...
  for (let i = 0; i < fileNames.length; i++) {
    ...
    if (/\.md|mdx/.test(fileNames[i])) {
      const convertFileToString = fs.readFileSync(
        path.resolve(__dirname, dir, fileNames[i]),
        "utf8"
      );
      const estimatedTimeToRead = Math.round(
        convertFileToString.split(" ").length / 250
      );
